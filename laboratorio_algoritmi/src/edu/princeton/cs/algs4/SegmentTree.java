...
public class SegmentTree {
	...
	private int rsq(int v, int from, int to) {
		...
		// If you did a range update that contained this node, you can infer the Sum
		// without going down the tree
	...
	private int rMinQ(int v, int from, int to) {
		...
		// If you did a range update that contained this node, you can infer the Min
		// value without going down the tree
		if (n.pendingVal != null && contains(n.from, n.to, from, to)) {
			return n.pendingVal;
		}
	 ...
	 * position or a range of positions with a given number. The update operations
	 * will update the less it can to update the whole range (Lazy Propagation). The
	 * values will be propagated lazily from top to bottom of the segment tree. This
	 * behavior is really useful for updates on portions of the array
	 * <p>
	 * Time-Complexity: O(log(n))
	 *
	 * @param from
	...
	private void update(int v, int from, int to, int value) {
		...
		// The Node of the heap tree represents a range of the array with bounds:
		// [n.from, n.to]
		Node n = heap[v];

		/**
		 * If the updating-range contains the portion of the current Node We lazily
		 * update it. This means We do NOT update each position of the vector, but
		 * update only some temporal values into the Node; such values into the Node
		 * will be propagated down to its children only when they need to.
		 */
		if (contains(from, to, n.from, n.to)) {
			change(n, value);
		}
		...
		if (intersects(from, to, n.from, n.to)) {
			/**
			 * Before keeping going down to the tree We need to propagate the the values
			 * that have been temporally/lazily saved into this Node to its children So that
			 * when We visit them the values are properly updated
			 */
			propagate(v);
	...
	/**
	 * Read the following commands: init n v Initializes the array of size n with
	 * all v's set a b c... Initializes the array with [a, b, c ...] rsq a b Range
	 * Sum Query for the range [a, b] rmq a b Range Min Query for the range [a, b]
	 * up a b v Update the [a,b] portion of the array with value v. exit
	 * <p>
	 * Example: init set 1 2 3 4 5 6 rsq 1 3 Sum from 1 to 3 = 6 rmq 1 3 Min from 1
	 * to 3 = 1 input up 1 3 [3,2,3,4,5,6]
	 *
	 * @param args
