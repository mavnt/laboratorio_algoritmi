 ...
 * <p>
 * This is an implementation of <em>Stoer–Wagner's algorithm</em> using an
 * index priority queue and the union-find data type in order to simplify
 * dealing with contracting edges. Precisely, the index priority queue is an
 * instance of {@link IndexMaxPQ} which is based on a binary heap. As a
 * consequence, the constructor takes <em>O</em>(<em>V</em> (<em>V</em> + <em>
 * E</em> ) log <em> V </em>) time and <em>O</em>(<em>V</em>) extra space (not
 * including the graph), where <em>V</em> is the number of vertices and
 * <em>E</em> is the number of edges. However, this time can be reduced to
 * <em>O</em>(<em>V E</em> + <em> V<sup>2</sup></em> log <em>V</em>) by using an
 * index priority queue implemented using Fibonacci heaps.
 * <p>
...
public class GlobalMincut {
	 ...
	 * cut-of-the-phase is a <em>minimum s-t-cut</em> in the current graph, where
	 * {@code s} and {@code t} are the two vertices added last in the phase.
	 */
	 ...
	 *             if the number of vertices of {@code G} is less than {@code 2} or
	 *             if anny edge weight is negative
	 */
	 ...
	 * vertices of the minimum cut; or {@code false} if the vertex {@code v} is on
	 * the second subset.
	 * 
	 * @param v
	 *            the vertex to check
	 * @return {@code true} if the vertex {@code v} is on the first subset of
	 *         vertices of the minimum cut; or {@code false} if the vertex {@code v}
	 *         is on the second subset.
	 * @throws IllegalArgumentException
	...
	/**
	 * Makes a cut for the current edge-weighted graph by partitioning its set of
	 * vertices into two nonempty subsets. The vertices connected to the vertex
	 * {@code t} belong to the first subset. Other vertices not connected to
	 * {@code t} belong to the second subset.
	 * 
	 * @param t
	 *            the vertex {@code t}
	 * @param uf
	...
	/**
	 * Computes a minimum cut of the edge-weighted graph. Precisely, it computes the
	 * lightest of the cuts-of-the-phase which yields the desired minimum cut.
	 * 
	 * @param G
	 *            the edge-weighted graph
	 * @param a
	...
	/**
	 * Returns the cut-of-the-phase. The cut-of-the-phase is a minimum s-t-cut in
	 * the current graph, where {@code s} and {@code t} are the two vertices added
	 * last in the phase. This algorithm is known in the literature as <em>maximum
	 ...
	 *            {@code true} if the vertex {@code v} was already contracted; or
	...
	/**
	 * Contracts the edges incidents on the vertices {@code s} and {@code t} of the
