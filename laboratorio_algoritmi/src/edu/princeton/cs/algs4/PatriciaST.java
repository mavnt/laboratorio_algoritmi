...
public class PatriciaST<Value> {
	...
	/*
	 * An inner Node class specifies the objects that hold each key-value pair. The
	 * b value indicates the relevant bit position.
	 */
	private class Node {
	 ...
	 */
	/*
	 * The constructor creates a head (sentinel) node that contains a zero-length
	 * string.
	 */
	public PatriciaST() {
	...
	/**
	 * Places a key-value pair into the symbol table. If the table already contains
	 * the specified key, then its associated value becomes updated. If the value
	 * provided is {@code null}, then the key becomes removed from the symbol table.
	 * 
	 * @param key
	 *            the key
	 * @param val
	 ...
	 * @param key
	 *            the key
	 * @return {@code true} if this symbol table contains the given {@code key} and
	 ...
	 *
	 * The inner loops of the get and put methods flow much better when they are not
	 * concerned with the lengths of strings, so a trick is employed to allow the
	 * get and put methods to view every string as an "infinite" sequence of bits.
	 * Logically, every string gets a '\uffff' character, followed by an "infinite"
	 * sequence of '\u0000' characters, appended to the end.
	 *
	 * Note that the '\uffff' character serves to mark the end of the string, and it
	 * is necessary. Simply padding with '\u0000' is insufficient to make all unique
	 * Unicode strings "look" unique to the get and put methods (because these
	 * methods do not regard string lengths).
	 */
	 ...
	 * look like an "infinite" sequence of characters. Logically, every string gets
	 * a '\uffff' character, followed by an "infinite" sequence of '\u0000'
	 ...
	 * characters first, and then considers the individual bits (once it finds two
	 * characters that do not match). Also, the least significant bits of an
	 * individual character are examined first. There are many Unicode alphabets
	 * where most (if not all) of the "action" occurs in the least significant bits.
	 *
	 * Notice that the very first character comparison excludes the
	 * least-significant bit. The firstDifferingBit function must never return zero;
	 * otherwise, a node would become created as a child to the head (sentinel) node
	 * that matches the bit-index value (zero) stored in the head node. This would
	 * violate the invariant that bit-index values increase as you descend into the
	 * trie.
	 */
	...
	/**
	 * Unit tests the {@code PatriciaST} data type. This test fixture runs a series
	 * of tests on a randomly generated dataset. You may specify up to two integer
	 * parameters on the command line. The first parameter indicates the size of the
	 * dataset. The second parameter controls the number of passes (a new random
	 * dataset becomes generated at the start of each pass).
	 *
	 * @param args
