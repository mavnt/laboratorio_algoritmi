...
public class DigraphGenerator {
	 ...
	 * @param E
	 *            the number of vertices
	 * @return a random simple digraph on {@code V} vertices, containing a total of
	 *         {@code E} edges
	...
	/**
	 * Returns a random simple digraph on {@code V} vertices, with an edge between
	 * any two vertices with probability {@code p}. This is sometimes referred to as
	 * the Erdos-Renyi random digraph model. This implementations takes time
	 ...
	 * digraph is a DAG in which for every two vertices, there is one directed edge.
	...
	/**
	 * Returns a random rooted-in DAG on {@code V} vertices and {@code E} edges. A
	 * rooted in-tree is a DAG in which there is a single vertex reachable from
	 * every other vertex. The DAG returned is not chosen uniformly at random among
	 * all such DAGs.
	 * 
	 * @param V
	 *            the number of vertices
	...
	/**
	 * Returns a random rooted-out DAG on {@code V} vertices and {@code E} edges. A
	 * rooted out-tree is a DAG in which every vertex is reachable from a single
	 * vertex. The DAG returned is not chosen uniformly at random among all such
	 * DAGs.
	 * 
	 * @param V
	 *            the number of vertices
	...
	/**
	 * Returns a random rooted-in tree on {@code V} vertices. A rooted in-tree is an
	 * oriented tree in which there is a single vertex reachable from every other
	 * vertex. The tree returned is not chosen uniformly at random among all such
	 * trees.
	 * 
	 * @param V
	 *            the number of vertices
	...
	/**
	 * Returns a random rooted-out tree on {@code V} vertices. A rooted out-tree is
	 * an oriented tree in which each vertex is reachable from a single vertex. It
	 * is also known as a <em>arborescence</em> or <em>branching</em>. The tree
	 * returned is not chosen uniformly at random among all such trees.
	 * 
	 * @param V
	 *            the number of vertices
	 ...
	 * @return a digraph that is a directed Eulerian cycle on {@code V} vertices and
	 *         {@code E} edges
	 * @throws IllegalArgumentException
	 ...
	 * @return a digraph that is a directed Eulerian path on {@code V} vertices and
	 *         {@code E} edges
	 * @throws IllegalArgumentException
	 *             if either {@code V <= 0} or {@code E < 0}
	 */
	...
	/**
	 * Returns a random simple digraph on {@code V} vertices, {@code E} edges and
	 * (at least) {@code c} strong components. The vertices are randomly assigned
	 * integer labels between {@code 0} and {@code c-1} (corresponding to strong
	 * components). Then, a strong component is creates among the vertices with the
	 * same label. Next, random edges (either between two vertices with the same
	 * labels or from a vetex with a smaller label to a vertex with a larger label).
	 ...
	 * @return a random simple digraph on {@code V} vertices and {@code E} edges,
	 *         with (at most) {@code c} strong components
	 * @throws IllegalArgumentException
