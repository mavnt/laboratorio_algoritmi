...
public class IndexFibonacciMinPQ<Key> implements Iterable<Integer> {
	 ...
	 * and {@code N-1} Worst case is O(n)
	 * 
	 * @param N
	...
	/**
	 * Changes the key associated with index i to the given key If the given key is
	 * greater, Worst case is O(log(n)) If the given key is lower, Worst case is
	 * O(1) (amortized)
	 * 
	 * @param i
	 *            an index
	...
	/**
	 * Decreases the key associated with index i to the given key Worst case is O(1)
	 * (amortized).
	 * 
	 * @param i
	 *            an index
	...
	/**
	 * Get an Iterator over the indexes in the priority queue in ascending order The
	 * Iterator does not implement the remove() method iterator() : Worst case is
	 * O(n) next() : Worst case is O(log(n)) (amortized) hasNext() : Worst case is
	 * O(1)
	 * 
