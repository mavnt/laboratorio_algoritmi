...
public class IndexMultiwayMinPQ<Key> implements Iterable<Integer> {
	...
	/**
	 * Initializes an empty indexed priority queue with indices between {@code 0} to
	 * {@code N-1} Worst case is O(n)
	 * 
	 * @param N
	 ...
	 * greater, Worst case is O(d*log-d(n)) If the given key is lower, Worst case is
	 * O(log-d(n))
	 * 
	 * @param i
	 *            an index
	 ...
	 * The Iterator does not implement the remove() method iterator() : Worst case
	 * is O(n) next() : Worst case is O(d*log-d(n)) hasNext() : Worst case is O(1)
	 * 
